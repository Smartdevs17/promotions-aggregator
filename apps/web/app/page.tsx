...
type PageResponse = { items: Promotion[]; pagination: { page: number; pageSize: number; totalItems: number; totalPages: number } };
type BrandsResponse = { items: BrandSummary[] };
type Run = { id?: string; runId?: string; jobId: string; state: string; attempted?: number; persisted?: number; updated?: number; failed?: number; sourceHealth?: string; checked?: number; discrepancyCount?: number; clean?: boolean; errorSummary?: string | null };
...
export default function HomePage() {
  ...
  async function loadPromotions(nextPage = page): Promise<void> {
    ...
    try { setData(await getJson<PageResponse>(`/promotions?${params}`)); }
    catch (err) { setError(err instanceof Error ? err.message : 'Unable to load promotions'); }
    finally { setLoading(false); }
  }

  useEffect(() => { void getJson<BrandsResponse>('/brands').then((value) => setBrands(value.items)).catch(() => setBrands([])); }, []);
  ...
  return <main>
    ...
    <nav className="pagination" aria-label="Pagination"><button disabled={page <= 1} onClick={() => setPage((v) => v - 1)}>← Previous</button><span>Page {data.pagination.page} of {Math.max(data.pagination.totalPages, 1)}</span><button disabled={page >= data.pagination.totalPages} onClick={() => setPage((v) => v + 1)}>Next →</button></nav>
  </main>;
}
